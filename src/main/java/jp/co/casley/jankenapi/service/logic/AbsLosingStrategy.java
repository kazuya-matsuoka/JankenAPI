 ...
 * 絶対敗北ストラテジー
 */
public class AbsLosingStrategy implements Strategy {
    /** ランダム値 */
    private Random random;
    /** じゃんけん組み合わせマスタ情報リスト */
    private List<MtJankenComb> mtJankenCombList;

    public AbsLosingStrategy(List<MtJankenComb> mtJankenCombList) {
    ...
    public String calcHand() {
        ...
        /* じゃんけん組み合わせマスタ情報リストからクライアントの出し手に負ける出し手を取得 */
        Optional<MtJankenComb> mtJankenComb = this.mtJankenCombList.stream()
                                                                               .filter(s->s.getResult().equals(ResultId.WIN.getId())).findFirst();

        /* 負けになる出し手を返却(該当なしの場合：ランダムに出し手を返却) */
